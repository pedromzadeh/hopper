...
def imshow_F_sigma(
    ...
    # superimpose given xv_traj onto phase space
    if xv_traj is not None:
        axs[2].scatter(xv_traj[:, 0], xv_traj[:, 1], s=50, color="purple")
...
def F_streamplot(
    ...
    fig1, ax = plt.subplots(1, 1, figsize=(5, 3.5), dpi=dpi)
    xmin, xmax, vmin, vmax, nbins = bounds
    ax.set_title(title["title"], fontsize=title["size"], y=1.1)
    ...
    _plot_trajs(F, bounds, stream_init_pts, ax, do_try, streamplot_kwargs)

    # superimpose given xv_traj onto phase space
    if xv_traj is not None:
        ax.scatter(xv_traj[:, 0], xv_traj[:, 1], s=50, color="purple")
...
def _plot_trajs(F, bounds, init_pts, ax, do_try, streamplot_kwargs):
    xmin, xmax, vmin, vmax, nbins = bounds
